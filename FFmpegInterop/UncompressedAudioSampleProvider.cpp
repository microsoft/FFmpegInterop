...
{
	UncompressedAudioSampleProvider::UncompressedAudioSampleProvider(_In_ const AVStream* stream, _Inout_ FFmpegReader& reader) :
	...
	tuple<IBuffer, int64_t, int64_t> UncompressedAudioSampleProvider::GetSampleData()
	{
		...
		{
			sampleData = make<FFmpegBuffer>(m_frame->buf[0]);
			sampleCount = m_frame->nb_samples;
		}
		else
		{
			// Resample the frame to the desired output format
			int result = swr_convert_frame(m_swrContext.get(), m_resampledFrame.get(), m_frame.get());
			if (result == AVERROR_INPUT_CHANGED)
			{
				// There was a format change. Reconfigure the resampler and try again.
				THROW_IF_FFMPEG_FAILED(swr_config_frame(m_swrContext.get(), m_resampledFrame.get(), m_frame.get()));
				THROW_IF_FFMPEG_FAILED(swr_init(m_swrContext.get()));
				THROW_IF_FFMPEG_FAILED(swr_convert_frame(m_swrContext.get(), m_resampledFrame.get(), m_frame.get()));
			}
			else
			{
			...
			sampleData = make<FFmpegBuffer>(m_resampledFrame->buf[0]);
			sampleCount = m_resampledFrame->nb_samples;

			av_frame_unref(m_resampledFrame.get());
		}

		// Calculate the pts and duration for the decoded sample
	...
	void UncompressedAudioSampleProvider::InitResamplerIfNeeded()
	{
	...
	tuple<IBuffer, int64_t, int64_t> UncompressedAudioSampleProvider::GetSampleData()
	{
		...
		else
		{
			...
			uint8_t* buf = nullptr;
			int bufSize = av_samples_alloc(&buf, nullptr, m_frame->channels, m_frame->nb_samples, AV_SAMPLE_FMT_S16, 0);
			THROW_IF_FFMPEG_FAILED(bufSize);

			int resampledSamplesCount = swr_convert(m_swrContext.get(), &buf, bufSize, m_frame->extended_data, m_frame->nb_samples);
			AVBlob_ptr resampledData{ exchange(buf, nullptr) };
			THROW_IF_FFMPEG_FAILED(resampledSamplesCount);

			const uint32_t resampledDataSize = resampledSamplesCount * m_frame->channels * av_get_bytes_per_sample(AV_SAMPLE_FMT_S16);
			WINRT_ASSERT(resampledDataSize <= bufSize);

			sampleData = make<FFmpegBuffer>(move(resampledData), resampledDataSize);
			sampleCount = resampledSamplesCount;
		}

		int64_t pts = m_frame->pkt_pts;
