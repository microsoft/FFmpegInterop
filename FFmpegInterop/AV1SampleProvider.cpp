...
{
	AV1SampleProvider::AV1SampleProvider(_In_ const AVFormatContext* formatContext, _In_ AVStream* stream, _In_ Reader& reader) :
		SampleProvider(formatContext, stream, reader)
	{

	}

	tuple<IBuffer, int64_t, int64_t, vector<pair<GUID, Windows::Foundation::IInspectable>>, vector<pair<GUID, Windows::Foundation::IInspectable>>> AV1SampleProvider::GetSampleData()
	{
		// Get the next sample
