...
HRESULT UncompressedAudioSampleProvider::AllocateResources()
{
	...
	if (SUCCEEDED(hr))
	{
		...
		int channels = m_pAvCodecCtx->profile == FF_PROFILE_AAC_HE_V2 ? m_pAvCodecCtx->channels * 2 : m_pAvCodecCtx->channels;
		int64 inChannelLayout = m_pAvCodecCtx->channel_layout && m_pAvCodecCtx->profile != FF_PROFILE_AAC_HE_V2 ? m_pAvCodecCtx->channel_layout : av_get_default_channel_layout(channels);
